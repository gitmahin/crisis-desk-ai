...
import {
  ReportRsrcPayloadInjectorMiddleware,
  ReportToolPaylodInjectorMiddleware,
} from "@/middlewares/reports.middleware";
...
router
  ...
  .post(
    ...
    asyncHandler(
      reportToolMiddleware.injectCreateReportPayload.bind(reportToolMiddleware)
    ),
...
router
  ...
  .get(
    asyncHandler(
      reportResourceMiddleware.injectGetReportByIdPayload.bind(
        reportResourceMiddleware
      )
    ),
    asyncHandler(useMCPResource),
...
router
  ...
  .get(
    asyncHandler(
      reportResourceMiddleware.injectGetAnalylticsPayload.bind(
        reportResourceMiddleware
      )
    ),
    asyncHandler(useMCPResource),
